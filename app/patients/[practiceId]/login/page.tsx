...
const LoginPage = ({ params: { practiceId } }: LoginPageProps) => {
  ...
  function PatientLoginForm() {
    ...
    async function onSubmit(values: { email: string; phone: string }) {
      ...
      try {
        // Find patient by email, phone, and practiceId
        const patient = await findPatientByEmailAndPhone(
          values.email,
          values.phone,
          practiceId
        );
        ...
        if (!patient) {
          setError(
            "Patient not found. Please check your credentials or register first."
          );
          setIsLoading(false);
          return;
        }
        ...
        // Login successful, redirect to appointment page with both patientId and practiceId
        router.push(
          `/patients/${practiceId}/new-appointment/${patient.userId}`
        );
      } catch (e) {
